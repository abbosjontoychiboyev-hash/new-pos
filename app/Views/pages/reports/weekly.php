<!-- Page Title -->
<?php $title = 'Haftalik hisobot - ' . date('d.m.Y', strtotime($start_date)) . ' - ' . date('d.m.Y', strtotime($end_date)); ?>
...
<div class="filter-section">
    <form method="GET" class="filter-row">
        <div>
            <label for="start_date" class="form-label fw-bold">Boshlanish sanasi:</label>
            <input type="date" id="start_date" name="start_date" value="<?= $start_date ?>" class="form-control" required>
        </div>
        <div>
            <label for="end_date" class="form-label fw-bold">Tugash sanasi:</label>
...
<div class="card-section">
    <div class="card-title">
        <i class="fas fa-calendar-week"></i> Kunlik tafsilotlar
    </div>
    <?php if (!empty($report['daily_breakdown'])): ?>
        <?php foreach ($report['daily_breakdown'] as $day => $dayData): ?>
        <div class="daily-breakdown">
            <h5><i class="fas fa-calendar-day"></i> <?= date('d.m.Y', strtotime($day)) ?> - <?= date('l', strtotime($day)) ?></h5>
            <div class="row">
                <div class="col-md-3">
                    <strong>Savdo:</strong> <?= number_format($dayData['gross_sales'], 0, ',', ' ') ?> so'm
                </div>
                <div class="col-md-3">
                    <strong>Qaytarish:</strong> <?= number_format($dayData['returns'], 0, ',', ' ') ?> so'm
                </div>
                <div class="col-md-3">
                    <strong>Qarz to'lovlari:</strong> <?= number_format($dayData['debt_collections'], 0, ',', ' ') ?> so'm
                </div>
                <div class="col-md-3">
                    <strong>Diller to'lovlari:</strong> <?= number_format($dayData['dealer_payments'], 0, ',', ' ') ?> so'm
                </div>
            </div>
        </div>
        <?php endforeach; ?>
    <?php else: ?>
        <div class="text-center py-4">
            ...
            <p class="text-muted">Kunlik tafsilotlar mavjud emas</p>
        </div>
    <?php endif; ?>
</div>
                ...
                <?php if (empty($report['sales']) || $report['sales']['total_sales'] == 0): ?>
                    <tr>
                        <td colspan="7" class="text-center py-4">
                            ...
                            <p class="text-muted">Bu haftada savdo bo'lmagan</p>
                        </td>
                    </tr>
                <?php else: ?>
                    <tr>
                ...
                <?php if (empty($report['dealer_payments']['payments'])): ?>
                    <tr>
                        <td colspan="6" class="text-center py-4">
                            ...
                            <p class="text-muted">Bu haftada diller to'lovlari bo'lmagan</p>
                        </td>
                    </tr>
                <?php else: ?>
                ...
                <?php if (empty($report['debt_collections']['collections'])): ?>
                    <tr>
                        <td colspan="6" class="text-center py-4">
                            ...
                            <p class="text-muted">Bu haftada qarz to'lovlari bo'lmagan</p>
                        </td>
                    </tr>
                <?php else: ?>
                ...
                <?php if (empty($report['returns']['returns'])): ?>
                    <tr>
                        <td colspan="6" class="text-center py-4">
                            ...
                            <p class="text-muted">Bu haftada qaytarishlar bo'lmagan</p>
                        </td>
                    </tr>
                <?php else: ?>
...
<div class="d-flex gap-2 justify-content-end">
    <a href="/new-pos/reports/export-excel/weekly?start_date=<?= $start_date ?>&end_date=<?= $end_date ?>&cashier_id=<?= $filters['cashier_id'] ?? '' ?>&dealer_id=<?= $filters['dealer_id'] ?? '' ?>" class="btn btn-success" title="Excel formatida yuklash">
        <i class="fas fa-file-excel"></i> Excel
    </a>
    <a href="/new-pos/reports/export-pdf/weekly?start_date=<?= $start_date ?>&end_date=<?= $end_date ?>&cashier_id=<?= $filters['cashier_id'] ?? '' ?>&dealer_id=<?= $filters['dealer_id'] ?? '' ?>" class="btn btn-danger" title="PDF formatida yuklash">
        <i class="fas fa-file-pdf"></i> PDF
    </a>
    <a href="/new-pos/reports/print/weekly?start_date=<?= $start_date ?>&end_date=<?= $end_date ?>&cashier_id=<?= $filters['cashier_id'] ?? '' ?>&dealer_id=<?= $filters['dealer_id'] ?? '' ?>" class="btn btn-info" title="Chop etish" target="_blank">
